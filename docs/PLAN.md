...
| 22 | 2026-09-07 | §9 | The header project switcher was a native `<select>` listing every project; `/admin` rendered every token row and every project row; there was no brand mark or favicon | The switcher is a progressive-enhancement combobox backed by `GET /projects/search?q=&limit=20`, with the `<select>` kept as a ≤20-option no-JS fallback; both `/admin` grids paginate server-side at ≤20 rows/page (`?tok_page=` / `?proj_page=`, independent, clamped); a monochrome SVG board mark is now the favicon and the header logo | The owner is dogfooding on installations that will hold hundreds of projects and many tokens, where shipping every row to the client on every page is both slow and a scaling cliff — and the product had no visual identity in the tab or the header | Additive, web layer only. The client never holds more than 20 projects/rows; the rest come from the server on search or page change. Vanilla JS only (CSP `script-src 'self'` forbids a library), monochrome preserved (the outcome badge and the logo reuse existing tokens), embedded/mirror asset trees kept byte-identical. Nine tools unchanged |
| 23 | 2026-09-07 | §5, §6 | A task carried no way to record that its result was wrong or unneeded once it was Done, and no post-hoc conclusion field distinct from body and notes | Added `outcome` (enum open/holds/refuted/superseded/moot, default open, independent of the column) and `conclusion` (free text). Migration `0004` adds both columns additively, with a CHECK on outcome that mirrors the enum | A model dogfooding the board on an all-night research program hit exactly this: it could move a task to Done but could not record that the conclusion later turned out refuted or moot, and wanted a verdict field kept apart from the running note log | Both flow through the existing `task_update` (nine tools unchanged), are replacement-style (require if_version, bump version) and validated (unknown outcome and oversized conclusion are refused). Reads hide the default outcome and the empty conclusion to keep board_get lean; compact renders a sparse `out <x>` only when not open, so the token budget is unmoved. "Comments under items" turned out to already exist as notes, so nothing was added there. HealthInfo.Migration reports 4 |
| 24 | 2026-09-07 | §5 | Default board = Backlog, Doing (WIP 3), Review, Done | Default is now three columns — **Backlog, Doing (WIP 3), Done**; Review is dropped from `domain.DefaultColumns` | The owner observed the Review column goes unused in practice, and a Codex review confirmed it earns no default space: `task_next(start)` always routes into the *first* active column (never Review), strict_done triggers on entering the done column (not Review), Review carries no WIP or special semantics, and the `reviewer` field is column-independent. Three columns reads as "the basic one" harder and matches the agent loop exactly (`task_next(start)` → Doing → `task_update(column:"Done")`) | Only `DefaultColumns` changed — arbitrary per-project columns stay built-in (`project_upsert` takes any set), so a Review column is one call away; "columns via plugins" was explicitly rejected as core config, not an extension. **Existing boards are NOT migrated** — columns are project data, so every current project keeps its exact layout; only a NEW project created without a column list gets three. Tests that exercise a second active column now declare their own Review column; docs/examples updated |
