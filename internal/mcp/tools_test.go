...
func TestListTools_FullServer_HasEleven(t *testing.T) {
	t.Parallel()
	...
	if len(res.Tools) != 11 {
		t.Errorf("tools count = %d, want 11 (got: %v)", len(res.Tools), names(res.Tools))
	}
	want := map[string]bool{
		...
		"project_post": false, "progress_set": false,
	}
